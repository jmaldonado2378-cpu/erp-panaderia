...
async function run() {
    console.log("Checking table 'charc_recetas'...");
    const { data, error } = await supabase.from('charc_recetas').select('*').limit(1);
    if (error) {
        console.error("Error fetching charc_recetas:", error);
    } else {
        console.log("charc_recetas columns keys:", data.length > 0 ? Object.keys(data[0]) : "No data in table");
    }
}
