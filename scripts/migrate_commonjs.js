...
const BASE_RECIPES = [
    { codigo: 'FG-F-001', nombre_producto: 'Baguette Francesa', familia: 'F', formato_venta: 'Unidad', peso_unidad: 250, merma: 18, horas_hombre: 1.5, costo_empaque: 0, lote_minimo: 1, unidad_lote: 'kg', details: [{ ingredientId: 'i1', porcentaje: 100, gramos: 1000 }, { ingredientId: 'i3', porcentaje: 65, gramos: 650 }, { ingredientId: 'i4', porcentaje: 2, gramos: 20 }, { ingredientId: 'i5', porcentaje: 1.5, gramos: 15 }, { ingredientId: 'i12', porcentaje: 1, gramos: 10 }] },
    { codigo: 'FG-F-002', nombre_producto: 'Pan de Molde Larga Vida', familia: 'F', formato_venta: 'Unidad', peso_unidad: 500, merma: 10, horas_hombre: 2.0, costo_empaque: 120, lote_minimo: 1, unidad_lote: 'kg', details: [{ ingredientId: 'i1', porcentaje: 100, gramos: 1000 }, { ingredientId: 'i3', porcentaje: 55, gramos: 550 }, { ingredientId: 'i6', porcentaje: 8, gramos: 80 }, { ingredientId: 'i5', porcentaje: 3, gramos: 30 }, { ingredientId: 'i13', porcentaje: 0.5, gramos: 5 }] },
    { codigo: 'FG-F-003', nombre_producto: 'Ciabatta Rústica', familia: 'F', formato_venta: 'Kg', peso_unidad: 0, merma: 15, horas_hombre: 2.5, costo_empaque: 0, lote_minimo: 1, unidad_lote: 'kg', details: [{ ingredientId: 'i1', porcentaje: 100, gramos: 1000 }, { ingredientId: 'wip_F1', porcentaje: 30, gramos: 300 }, { ingredientId: 'i3', porcentaje: 80, gramos: 800 }, { ingredientId: 'i4', porcentaje: 2.2, gramos: 22 }] },
];
...
async function run() {
    ...
    for (const ing of INITIAL_INGREDIENTS) {
        ...
        if (error) {
            ...
            mappedIngredients[ing.id] = data[0].id;
        }
    }
    ...
    for (const rec of BASE_RECIPES) {
        ...
        const { data, error } = await supabase.from('recetas').insert([{
            ...
            lote_minimo: rec.lote_minimo,
            unidad_lote: rec.unidad_lote
        }]).select();
