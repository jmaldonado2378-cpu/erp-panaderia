        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-left animate-in fade-in">
            <div className="lg:col-span-2 space-y-6">
                <Card className="p-8 border-t-8 border-blue-600 bg-white shadow-xl">
                    <h4 className="text-2xl font-black uppercase italic mb-8 border-b pb-4 flex items-center gap-3 text-slate-800"><Truck className="text-blue-600" size={28} /> Armar Remito de Traslado</h4>
                    <div className="mb-8">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Local Destino</label>
                        <select className="w-full border-2 rounded-xl p-3 font-bold bg-slate-50 outline-none text-sm" value={destination} onChange={e => setDestination(e.target.value)}>
                            <option value="">Elegir Sucursal...</option>
                            {branches?.map(b => <option key={b} value={b}>{b}</option>)}
                        </select>
                    </div>
                    <div className="flex gap-4 items-end bg-slate-50 p-6 rounded-2xl border mb-8">
                        <div className="flex-1">
                            <Select label="Producto Terminado" value={currentItem.recipeId} onChange={e => setCurrentItem({ ...currentItem, recipeId: e })}>
                                <option value="">Seleccionar...</option>
                                {recipes.filter(r => !r.es_subensamble).map(r => <option key={r.id} value={r.id}>{r.nombre_producto}</option>)}
                            </Select>
                        </div>
                        <div className="w-32">
                            <Input label="Cantidad" type="number" value={currentItem.amount} onChange={v => setCurrentItem({ ...currentItem, amount: v })} />
                        </div>
                        <Button variant="primary" className="py-2.5 h-[42px]" onClick={addToCart}>Agregar</Button>
                    </div>
                    {cart.length > 0 && (
                        <div className="space-y-4 animate-in slide-in-from-bottom-4">
                            <table className="w-full text-left border rounded-xl overflow-hidden"><thead className="bg-slate-900 text-white text-[9px] uppercase tracking-widest"><tr><th className="px-3 py-1.5">Producto</th><th className="px-3 py-1.5 text-right">Cantidad</th><th className="px-3 py-1.5 text-center">Acción</th></tr></thead><tbody className="divide-y divide-slate-100 bg-white font-bold text-xs">{cart.map((item, idx) => (<tr key={idx}><td className="px-3 py-1.5 uppercase text-slate-800">{item.nombre_producto}</td><td className="px-3 py-1.5 text-right text-blue-600 font-mono">{item.amount} U</td><td className="px-3 py-1.5 text-center"><button onClick={() => { const newCart = [...cart]; newCart.splice(idx, 1); setCart(newCart); }} className="text-red-400 hover:text-red-600 p-1"><Trash2 size={12} className="mx-auto" /></button></td></tr>))}</tbody></table>
                            <Button variant="success" className="w-full py-4" onClick={saveDispatch}>Emitir Remito Oficial</Button>
                        </div>
                    )}
                </Card>
            </div>
                            ...
                            <tbody className="divide-y divide-slate-100 bg-white">
                                {ingredients.map(i => (
                                    <tr key={i.id} className="hover:bg-slate-50 transition-colors group">
                                        ...
                                        <td className="px-4 py-1.5 text-center">
                                            {i.alergeno ? <span className="bg-red-50 text-red-600 border border-red-100 px-1.5 py-0.5 rounded text-[9px]">{i.alergeno}</span> : <span className="text-slate-300">-</span>}
                                        </td>
